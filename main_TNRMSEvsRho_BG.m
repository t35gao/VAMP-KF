...
rho_BCN_options = 0:0.1:0.9;            % sparsity rho (rate of zeros) 

% NRMSE storage matrices
size_storage    = [length(rho_BCN_options), N_MC, N_time];
...
for i_rho_BCN = 1:length(rho_BCN_options)
    ...
    for i_MC = 1:N_MC
        ...
        for t = 1:N_time
            ...
            NRMSE_x_KF(i_rho_BCN, i_MC, t) = getNRMSE(xht_KF, x);
            % ----------- EnKF -----------
            [xht_EnKF, X_EnKF] = EnKF(F, H, y, Q_Gv, R_w, X_old_EnKF, N_EnKF);
            NRMSE_x_EnKF(i_rho_BCN, i_MC, t) = getNRMSE(xht_EnKF, x);
            % ---------- MCC-KF ----------
            [xht_MCCKF, Sht_MCCKF] = MCCKF(F, H, y, Q_Gv, R_w, xht_old_MCCKF, Sht_old_MCCKF);
            NRMSE_x_MCCKF(i_rho_BCN, i_MC, t) = getNRMSE(xht_MCCKF, x);
            % --------- VAMP-KF ----------
            [xht_VAMPKF, Sht_VAMPKF] = algo_3_VAMPKF(sysPkg_VAMPKF, y, R_w, leaf_v1, itPrams);
            NRMSE_x_VAMPKF(i_rho_BCN, i_MC, t) = getNRMSE(xht_VAMPKF, x);
            % -------- update old --------
            xht_old_KF      = xht_KF;
            Sht_old_KF      = Sht_KF;
            X_old_EnKF      = X_EnKF;
            ...
            % displaying NRMSE
            fprintf(1, "[rho = %0.1f][MC = %03d][t = %03d]: ", rho_BCN, i_MC, t);
            fprintf(1, "e_KF = %8.5f"      + ", " + ...
                       "e_EnKF = %8.5f"    + ", " + ...
                       "e_MCC-KF = %8.5f"  + ", " + ...
                       "e_VAMP-KF = %8.5f" + "\n" , ...
                       NRMSE_x_KF(i_rho_BCN, i_MC, t), ...
                       NRMSE_x_EnKF(i_rho_BCN, i_MC, t), ...
                       NRMSE_x_MCCKF(i_rho_BCN, i_MC, t), ...
                       NRMSE_x_VAMPKF(i_rho_BCN, i_MC, t));
        end
    end
end
...
semilogy(rho_BCN_options, TNRMSE_x_KF, 'DisplayName', 'standard KF', ...
    'LineWidth', 3, 'Color', colors(4,:)); hold on;
errorbar(rho_BCN_options, TNRMSE_x_KF, TNRMSE_x_KF_ste/2, 'o', ...
...
semilogy(rho_BCN_options, TNRMSE_x_EnKF, 'DisplayName', 'EnKF', ...
    'LineWidth', 3, 'Color', colors(3,:)); hold on;
errorbar(rho_BCN_options, TNRMSE_x_EnKF, TNRMSE_x_EnKF_ste/2, 'o', ...
...
semilogy(rho_BCN_options, TNRMSE_x_MCCKF, 'DisplayName', 'MCC-KF', ...
    'LineWidth', 3, 'Color', colors(2,:)); hold on;
errorbar(rho_BCN_options, TNRMSE_x_MCCKF, TNRMSE_x_MCCKF_ste/2, 'o', ...
...
semilogy(rho_BCN_options, TNRMSE_x_VAMPKF, 'DisplayName', 'VAMP-KF', ...
    'LineWidth', 3, 'Color', colors(1,:)); hold on;
errorbar(rho_BCN_options, TNRMSE_x_VAMPKF, TNRMSE_x_VAMPKF_ste/2, 'o', ...
...
ca.XLim     = rho_BCN_options([1, end]);
ca.XTick    = rho_BCN_options;

% case of underdetermined observation matrix
if Ny < Nx; ca.YLim(2) = 1; end
