...
rho_BCN         = 0.9;                  % sparsity rho (rate of zeros)
...
L2M         = Nv / Ny;
N2M         = Nx / Ny;
LvarG       = (1-alp^2) * L2M / N2M;
MvarH       = 1;
NvarF       = alp^2;
NvarG       = 1-alp^2;
NvarH       = N2M;

% NRMSE storage matrix for VAMP-KF SE
NRMSE_x_SE  = zeros(length(SNR_options), N_time);

for i_SNR = 1:length(SNR_options)
    SNR             = SNR_options(i_SNR);
    % asymptotic signal power
    power_Hx_SE     = N2M * power_x;
    % asymptotic measurement noise precision
    gma_w_SE        = 10^(SNR/10) / power_Hx_SE;

    % SE at t = 0
    gma_xht_old_SE  = 1 / mean(diag(Sht_init));
    % packaging SE inputs
    sysPkg_SE       = SysPkg_SE(L2M, N2M, LvarG, MvarH, NvarF, NvarG, NvarH, gma_xht_old_SE);
    ...
    for t = 1:N_time
        ...
        %----------------------------%
        % -------- VAMP-KF SE --------
        [gma_xht_SE]            = algo_SE3_VAMPKF(sysPkg_SE, gma_w_SE, leaf_v1, itPrams);
        NRMSE_x_SE(i_SNR, t)    = sqrt((1/gma_xht_SE) / power_x);
        % -------- update old --------
        gma_xht_old_SE          = gma_xht_SE;
        sysPkg_SE               = sysPkg_SE.updateOld(gma_xht_old_SE);
        

        % displaying NRMSE
        fprintf(1, "[SNR = %02d][t = %03d]: ", SNR, t);
        fprintf(1, "e_VAMP-KF_SE = %8.5f\n", NRMSE_x_SE(i_SNR, t));
    end
end
...
TNRMSE_x_SE_ste = TNRMSE_x_SE_std / sqrt_N_sample;
